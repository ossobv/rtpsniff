...
void out_write(uint32_t unixtime_begin, uint32_t interval, struct rtpstat_t *memory) {
    ...
    HASH_ITER(hh, memory, rtpstat, tmp) {
        ...
        syslog(LOG_NOTICE,
               ...
               "late_or_dupe=%" PRIu16 " latepct=%.2f "
               ...
               rtpstat->packets,
               rtpstat->missed, /* lost */
               100.0 * rtpstat->missed / (rtpstat->missed + rtpstat->packets),
               rtpstat->late,
               100.0 * rtpstat->late / (rtpstat->missed + rtpstat->packets),
               rtpstat->gaps,
               rtpstat->jumps,
               rtpstat->seq);
    }
    ...
    if (!packets) {
        ...
        syslog(LOG_NOTICE, "streams=%u not_lost=%u lost=%u lostpct=%.2f late_or_dupe=%u latepct=%.2f",
               streams, packets, lost, 100.0 * lost / (packets + lost),
